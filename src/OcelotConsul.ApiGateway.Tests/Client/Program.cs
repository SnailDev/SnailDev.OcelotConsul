        ...
        static void GetString(HttpClient client)
        {
            while (true)
            {
                ...
                try
                {
                    ...
                    {
                        case "1":
                            result = client.GetStringAsync("http://localhost:5000/api001/values").GetAwaiter().GetResult();
                            break;
                        case "2":
                            result = client.GetStringAsync("http://localhost:5000/api002/values").GetAwaiter().GetResult();
                            break;
                        case "e":
                            return;
                    }
